...
const InputField = ({ label, name, type = "text", placeholder, value, onChange }) => (
  <div>
    ...
    <input
      ...
      value={value || ""}
      onChange={onChange}
      className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 text-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white placeholder:text-gray-400"
  ...
  return (
    <div className="mb-10 p-8 bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
        ...
        <InputField label="Email Address" name="email" type="email" placeholder="john@example.com" value={state.email} onChange={handleBasicInfoChange} />
        <InputField label="Phone Number" name="phone" placeholder="(+1) 234 567 890" value={state.phone} onChange={handleBasicInfoChange} />
        <InputField label="LinkedIn URL" name="linkedin" type="url" placeholder="linkedin.com/in/johndoe" value={state.linkedin} onChange={handleBasicInfoChange} />
      </div>
